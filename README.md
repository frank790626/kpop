...
一個 K-POP 團體介紹網站，目前收錄 **BABYMONSTER**、**IU**、**LE SSERAFIM**、**IVE**、**aespa**、**ILLIT**、**TWS**、**NMIXX**、**太妍 TAEYEON**、**QWER**、**ATEEZ**，
...
團體頁首、成員卡、影片卡都有愛心按鈕，收藏會存在這台裝置瀏覽器的 localStorage（`kpop:favorites:v1`），
不用登入；右上角「我的最愛」連到 `/kpop/favorites/`（不給搜尋引擎收錄）。
...
- 收藏頁每一格左邊的 ⠿ 可以拖曳自訂排序（滑鼠、手指、鍵盤空白鍵＋方向鍵都行），順序會跟著雲端同步。
